...
def pre_emhpasis(alpha):
    global sig
    emphasized_signal = np.append(sig[0], sig[1:] - alpha * sig[:-1])
...
def process(emphasized_signal):
    ...
    for i in range(frame_num):
        ...
        NFFT = sample_width
        mag_frames.append(np.absolute(np.fft.fft(temp, NFFT)))

        pow_frames.append(
            (1.0/NFFT)*((np.absolute(np.fft.fft(temp, NFFT)))**2))
...
def mel(frames, NFFT):
    ...
    fbank = np.zeros((mel_filter_channels, int(np.floor(NFFT
    ))))
